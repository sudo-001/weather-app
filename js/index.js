...
function getCoordonates(name) {
    fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${name}&limit=10&appid=${apiKey}`)
        ...
        .then(data => {
            isLoading = true;
            loaderWrapper.style.display = "inherit";
            loader.style.animation = "rotate infinite 2s linear";
            coordonates = data;
            console.log(coordonates);
            getWeatherData(coordonates[0].lat, coordonates[0].lon);
        })
        .catch(err => {
            if (err.message.match(/Cannot read properties of undefined/)) {
                loader.style.animation = "";
                loaderWrapper.style.display = "none";
                alert("Il se pourrais que cette ville n'existe pas ");
            }
            console.log({ err })
        })
}
